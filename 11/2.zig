...
const Monkey = struct {
    ...
    fn through_next(self: *Self) ?u128 {
    ...
    fn get_target(self: Self, item: u128) usize {
...
pub fn main() !void {
    ...
    while (try in_stream.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        if (std.mem.startsWith(u8, line, "Monkey")) {
            try monkeys.append(Monkey{ .items = std.ArrayList(u128).init(allocator) });
            ...
            while (iterator.next()) |item| {
                try new_monkey.?.items.append(std.fmt.parseInt(u128, item, 10) catch unreachable);
            }
            ...
            if (std.mem.containsAtLeast(u8, line, 2, "old")) {
                ...
                new_monkey.?.value = try std.fmt.parseInt(u128, line[25..], 10);
            ...
            new_monkey.?.test_value = try std.fmt.parseInt(u128, line[21..], 10);
    ...
    while (round < 10000) : (round += 1) {
        ...
        for (monkeys.items) |*monkey| {
            while (monkey.through_next()) |item| {
                ...
                try monkeys.items[target].items.append(@rem(item, limit));
            }
        }
    }
