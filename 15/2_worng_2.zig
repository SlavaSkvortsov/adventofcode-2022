...
pub fn main() !void {
    ...
    while (try in_stream.readUntilDelimiterOrEof(&buf, '\n')) |line| {
        ...
        // top and right
        try lines.append(
            try make_line_by_two_points(
                Coords{ .x = sensor_coords.x, .y = sensor_coords.y + distance},
                Coords{ .x = sensor_coords.x + distance, .y = sensor_coords.y},
            ),
        );
        // top and left
        try lines.append(
    ...
    for (lines.items) |line| {
        for (lines.items) |other_line| {
            if (line.a == other_line.a and line.b == other_line.b) continue;
            if (try get_intersection(line, other_line)) |intersection| {
                // print("intersection: ({}, {})\n", .{intersection.x, intersection.y});
                try points.put(intersection, 0);
                if (try scan_near_point(intersection, sensors)) {
                    print("Found point at {},{}\n", .{intersection.x, intersection.y});
                    return;
                }
            }
