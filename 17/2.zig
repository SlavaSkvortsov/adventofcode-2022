...
pub fn main() !void {
    ...
    // var j: u64 = 0;
    // var prev_max_y = chamber.max_y;
    // var prev_i: usize = 0;
    // var i: usize = 0;
    // while (j < 20) : (j += 1) {
    //     var first = true;
    //     while (wind.cursor != 0 or first) : (i += 1) {
    ...
    //     print("diff i = {}\n", .{i - prev_i});
    //     print("max_y = {}\n", .{chamber.max_y});
    //     print("diff = {}\n", .{chamber.max_y - prev_max_y});
    //     print("----------------\n", .{});
    //     prev_max_y = chamber.max_y;
    //     prev_i = i;
    // }


    const rocks_first_cycle = 1757;
    const rocks_per_cycle = 1755;
    const gain_per_cycle: u16 = 2747;
    const total_rocks: u128 = 1000000000000;  // 46932 for 30000 stones
    const remaining_rocks_tmp = total_rocks - rocks_first_cycle - rocks_per_cycle;
    const remaining = @mod(remaining_rocks_tmp, rocks_per_cycle);
    var i: usize = 0;
    while (i < remaining + rocks_first_cycle + rocks_per_cycle) : (i += 1) {
        drop_figure(figures[i % figures.len], &wind, &chamber);
    }
    print("max_y = {}\n", .{chamber.max_y + try std.math.divTrunc(u128, remaining_rocks_tmp, rocks_per_cycle) * gain_per_cycle });
    ...
    // 1549231646384 is too low
    // 1549231644850 is too low
    // 1548350399700 is too low
    // 1565242165201 - !
}
